...
select DEPARTMENT_ID,count(*) as TotalEmployees,
  ...
  cast(sum(case when datediff(year, HIRE_DATE, GETDATE()) > 3 then 1 else 0 end) * 100 / count(*) as DECIMAL(5,2)) as PercentageOver3Years
from Employees
...
SELECT 
    id,
    COALESCE(ssn, passportid, itin) AS non_null
FROM person;
...
select JobDescription,
       max(SpacemanID) as MostExperiencedID,
       min(SpacemanID) as LeastExperiencedID
from (select JobDescription,SpacemanID,
        rank() over (partition by JobDescription order by names) as ExperienceRank,
        rank() over (partition by JobDescription order by names desc) as LeastRank
    from Personal
) as Ranked
where ExperienceRank = 1 or LeastRank = 1
